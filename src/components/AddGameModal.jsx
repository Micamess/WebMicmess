...
const emptyForm = { title: '', cover_url: '', status: 'jugando', hours_played: 0, note: '' }

export default function AddGameModal({ onClose, onCreate }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
      <div className="max-h-[88vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-white/10 bg-bg-mid p-5 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="font-display text-lg font-semibold">Agregar juego</h3>
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <div className="flex gap-3">
            <select
              ...
              className="flex-1 rounded-md border border-white/10 bg-bg-panel px-3 py-2 text-sm"
            >
            ...
            <input
              ...
              onChange={(e) => setForm((f) => ({ ...f, hours_played: e.target.value }))}
              placeholder="Horas"
              className="w-28 rounded-md border border-white/10 bg-bg-panel px-3 py-2 text-sm placeholder:text-ink-faint"
            />
          </div>
